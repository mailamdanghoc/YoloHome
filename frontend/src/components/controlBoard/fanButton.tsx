...
const FanButton = (): ReactNode => {
    ...
    const handleSetFanSpeed = (event: React.FormEvent): void => {
        ...
        const newSpeed = (Number((event.target as HTMLInputElement).value) + 1) * 25;
    ...
    return (
        <div className="h-full w-full p-6 rounded-xl bg-white">
            <div className="h-1/4 w-full flex ites">
                <PiFan size={35} className={`${(data ? data.speed / 25 - 1 : 0) != 0 ? "w-0" : "sm:w-fit w-0"} text-gray-300`} />
                <PiFanFill size={35} className={`${(data ? data.speed / 25 - 1 : 0) != 0 ? "sm:w-fit w-0" : "w-0"} text-blue-500`} />
            ...
            <div className="h-3/4 sm:h-1/2 w-full flex items-center justify-center lg:justify-start">
                ...
                [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-[100%] [&::-webkit-slider-thumb]:w-1/4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gray-500"
                    ...
                    value={(data ? data.speed / 25 - 1 : 0).toString()}
                />
            </div>
            <div className="h-1/4 w-full hidden items-center sm:flex">
                <span className={`w-1/4 text-center font-semibold text-base lg:text-xl ${(data ? data.speed / 25 - 1 : 0) == 0 ? "text-cyan-400" : "text-gray-500"}`}>Off</span>
                <span className={`w-1/4 text-center font-semibold text-base lg:text-xl ${(data ? data.speed / 25 - 1 : 0) == 1 ? "text-cyan-400" : "text-gray-500"}`}>Low</span>
                <span className={`w-1/4 text-center font-semibold text-base lg:text-xl ${(data ? data.speed / 25 - 1 : 0) == 2 ? "text-cyan-400" : "text-gray-500"}`}>Medium</span>
                <span className={`w-1/4 text-center font-semibold text-base lg:text-xl ${(data ? data.speed / 25 - 1 : 0) == 3 ? "text-cyan-400" : "text-gray-500"}`}>High</span>
            </div>
        </div>
    )
}
